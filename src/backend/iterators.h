...
namespace inspire {
  namespace backend {
    ...
    class ProteinIterator {
      ...
      virtual std::string getBasicModelName(std::string model) = 0;
      virtual std::string getBasicChainName(std::string chain) = 0;

      virtual std::string getFullModelName(std::string model) = 0;
      virtual std::string getFullChainName(std::string chain) = 0;
    ...
    // This iterator use only the first model, the first biomolecule, the first biomolecule transformation, the first crystallographic transformation and the first alternative location
    // and iterates through chains and aminoacids only
    class ExplicitIterator : public ProteinIterator {
      ...
      // Returns '1'
      std::string getBasicModelName(std::string model) override {
        if (model.empty()) {
          return "1";
        }
        throw common::exception::TitledException("'" + model + "' is not configuration generated by this iterator");
      };
      // Returns 'chainID'
      std::string getBasicChainName(std::string chain) override {
        return chain;
      }

      // Returns ''
      std::string getFullModelName(std::string model) override {
        if (model.empty()) {
          return "";
        }
        throw common::exception::TitledException("'" + model + "' is not configuration generated by this iterator");
      ...
      // Returns 'chainID'
      std::string getFullChainName(std::string chain) override {
        return chain;
      }
    ...
    class FirstModelIterator : public ProteinIterator {
      ...
      // Removes part startign with '+'
      std::string getBasicChainName(std::string chain) override {
        size_t end = chain.find('+');
        if (end == chain.npos) {
          return chain;
        }
    ...
    class FirstModelCrystallographicIterator : public ProteinIterator {
      ...
      // Removes part startign with '+', resp. '*'
      std::string getBasicChainName(std::string chain) override {
        size_t endp = chain.find('+');
        size_t endt = chain.find('*');
        if (endp == chain.npos) {
          if (endt == chain.npos) {
            return chain;
          }
          return chain.substr(0, endt);
        } else if (endt == chain.npos || endp < endt) {
          return chain.substr(0, endp);
        }
        return chain.substr(0, endt);
      }
      ...
      std::string getFullChainName(std::string chain) override {
        ...
        size_t endt = chain.find('*');
        if (endt == chain.npos) {
          if (endp == chain.npos) { // chainId
            return chain + "+1*1";
          } else { // chainId+assemblyTransformationID
            return chain + "*1";
          }
        } else {
          if (endp == chain.npos) { // chainId*crystallographicTransformationID
            return chain.substr(0, endt) + "+1" + chain.substr(endt);
          } else if (endp < endt) { // chainId+assemblyTransformationID*crystallographicTransformationID
            return chain;
          } else { // chainId*crystallographicTransformationID+assemblyTransformationID
            return chain.substr(0, endt) + chain.substr(endp) + chain.substr(endt, endp-endt);
          }
        }
      }
    ...
    class BiomoleculesIterator : public ProteinIterator {
      ...
      // Returns part after ',' and before ';'
      std::string getBasicModelName(std::string model) override {
        size_t start = model.find(',');
        if (start == model.npos) {
          return "1";
        }
        size_t end = model.find(';');
        if (end == model.npos || end < start) {
          return model.substr(start+1);
        }
        return model.substr(start+1, end-start-1);
      };
      // Removes part startign with '+'
      std::string getBasicChainName(std::string chain) override {
        size_t end = chain.find('+');
      ...
      // Returns ;'biomoleculeID','modelID'
      std::string getFullModelName(std::string model) override {
        size_t startc = model.find(',');
        size_t starts = model.find(';');
        if (startc == model.npos) {
          if (starts == model.npos) { // ''
            return ";1,1";
          } else { // ;biomoleculeID
            return model + ",1";
          }
        } else {
          if (starts == model.npos) { // ,modelID
            return ";1" + model;
          } else if (starts < startc) { // ;biomoleculeID,modelID
            return model;
          } else { // ,modelID;biomoleculeID
            return model.substr(starts) + model.substr(0, starts);
          }
        }
      };
      // Returns 'chainID'+'assemblyTransformationID'
