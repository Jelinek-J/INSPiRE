  ...
  namespace backend {
    struct ProteinParser {
      static std::string parse_id(const std::string& input) {
        ...
        if (common::string::ends_with(input, ".pdb")) {
          ...
          throw common::exception::TitledException(input + ": PDBML file format is not currently supported"); 
#else
          return Xml::parse_id(stream);
#endif
        } else {
// TODO: Try to recognize format based on the first line
          throw common::exception::TitledException(input + ": Unknown file format is not currently supported");
        }
      }
